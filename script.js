/*
  script.js
  Ranking dinámico con:
  - Orden automático por puntos (desc)
  - Empates (puntos iguales)
  - Subidas/bajadas calculadas automáticamente SIN mantener previousPosition a mano

  ¿Cómo se calcula la “previousPosition”?
  - Guardamos en localStorage la posición de cada atleta por su id.
  - En la siguiente carga, esa posición guardada se usa como “posición anterior”.
  - Si cambias los puntos en el código, el orden cambia y se comparará contra la última posición guardada.

  Nota:
  - Si quieres “reiniciar” el histórico (que nadie aparezca como subido/bajado), borra el localStorage
    del navegador o cambia la clave STORAGE_KEY.
*/
...
// =========================
// Datos (edita SOLO puntos/nombres/redes; no hay previousPosition manual)
// =========================
...
const athletes = [
  {
    ...
    socials: { instagram: "https://www.instagram.com/paracu.sw/", youtube: "https://youtube.com/@paracu_sw?si=YCiXktwlxgzpzq3P", tiktok: "https://www.tiktok.com/@user691637720?_r=1&_t=ZN-942DjwGJtcU" },
...
function getFilteredRanked() {
  ...
  const filtered = ranked.filter(a => {
    if (!q) return true;
    // Si el usuario escribe SOLO una posición (ej. 30), mostramos desde esa posición en adelante.
    if (jumpPos !== null) return a.position >= jumpPos;
    // Si no, búsqueda normal por nombre o por coincidencia parcial en posición.
...
function makeAvatarDataUri(name) {
  // Avatar local (sin recursos remotos)
  const initials = name
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
...
function computeRanking() {
  // Orden por puntos desc. Si hay empate, por nombre (estable).
  const sorted = [...athletes].sort((a, b) => {
    if (b.points !== a.points) return b.points - a.points;
    return a.name.localeCompare(b.name, "es");
  });

  // Empates: contar repeticiones de puntos
  ...
  ranked = sorted.map((a, idx) => {
    ...
    // Posición anterior: leemos del storage; si no existe, asumimos la actual
...
// Guardamos el ranking actual para que la próxima vez sea la “previousPosition”
